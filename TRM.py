...
## 7. ScaledDotProductAttention注意力机制
...
## 5. EncoderLayer ：包含两个子层，多头注意力机制和前馈神经网络
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(src_vocab_size, d_model)  ## 这个其实就是去定义生成一个矩阵，大小是 src_vocab_size * d_model（每个词代表了一个行向量）
        self.pos_emb = PositionalEncoding(d_model) ## 位置编码情况，这里是固定的正余弦函数，也可以使用类似词向量的nn.Embedding获得一个可以更新学习的位置编码
        self.layers = nn.ModuleList([EncoderLayer() for _ in range(n_layers)]) ##列表形式： 使用ModuleList对多个encoder进行堆叠，因为后续的encoder并没有使用词向量和位置编码，所以抽离出来；
        #EncoderLayer：编码器的总层
    def forward(self, enc_inputs):
        ## 这里我们的 enc_inputs 形状是： [batch_size x source_len]:source_len是原句子长度

        ## 下面这个代码通过src_emb，进行索引定位，enc_outputs输出形状是[batch_size, src_len, d_model]：原本是一定批量的句子，输入之后将词转化为向量从而使得原有句子变成了行为词数量，列为统一维度的矩阵，再加上批量形成了3D张量的形式！！！
        ...
        for layer in self.layers:
            ...
            enc_outputs, enc_self_attn = layer(enc_outputs, enc_self_attn_mask)#将enc_outputs, enc_self_attn_mask放入EncoderLayer后得到enc_outputs, enc_self_attn，再利用新的enc_outputs与enc_self_attn_mask进行迭代
...
## 1. 从整体网路结构来看，分为三个部分：编码层，解码层，输出层，再利用forward将三个部分进行连接！
class Transformer(nn.Module):
    def __init__(self):
        super(Transformer, self).__init__()
        self.encoder = Encoder()  ## 编码层（自定义类）
        self.decoder = Decoder()  ## 解码层（自定义类）
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False) ## 输出层 d_model 是我们解码层每个向量输出的维度大小，之后会做一个 tgt_vocab_size（词表大小） 大小的softmax
    def forward(self, enc_inputs, dec_inputs): #在所定义的三个层中一共输入两个参数
        ## 这里有两个数据进行输入，一个是enc_inputs 形状为[样本个数, 特征维度]，主要是作为编码段的输入，一个dec_inputs，形状为[样本个数, 特征维度]，主要是作为解码端的输入
...
if __name__ == '__main__':
    ...
    model = Transformer()#调用transformer模型
